...
class RatesUpdater():
  ...
  def run_update(self):
    ...
    action_logger.info(f"Starting rates update...", extra={'action': LOG_ACTION_API})

    for client in self.clients:
      try:

        res = client.fetch_rates()
        action_logger.info(f"Fetching rates from {client.__class__.__name__}... OK ({len(res)} rates)", extra={'action': LOG_ACTION_API})
